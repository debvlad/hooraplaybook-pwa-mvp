...
const APP_HEADER = `<img class="brand-mark-img" src="/assets/hooraplaybook-logo.png" alt=""><div class="logo">${APP_LOGO}</div>`;
...
const MATERIAL_OPTIONS = ['No Materials', 'Cups', 'Paper', 'Balls', 'Chairs', 'Rope', 'Balloons', 'Cones', 'Tape'];
...
const FILTER_SECTIONS = [
  ...
  { title: 'Materials', key: 'materials', options: MATERIAL_OPTIONS },
...
function loadState() {
  ...
  const initial = {
    ...
    sort: 'Best Match',
    search: '',
    finder: { groupSize: 14, ageMin: 12, ageMax: 18, time: 20, space: 'indoor', materials: 'No Materials', category: 'Quick and simple', safety: 'Low Risk', avoid: 'not embarrassing' }
  };
  saveState(initial);
  return initial;
}
...
function go(path) {
  ...
  if (route?.raw && route.raw !== path) window.__routeStack.push(route.raw);
  window.__pendingScrollTop = true;
  window.location.hash = path;
}

function goBack() {
  const stack = window.__routeStack || [];
  const previous = stack.pop();
  if (previous && previous !== route.raw) {
    window.__pendingScrollTop = true;
    window.location.hash = previous;
  } else if (history.length > 1) {
    history.back();
  } else {
    window.location.hash = '/app/find';
  }
}
...
function render() {
  ...
  else root.innerHTML = renderPrivateRoute() + renderAddToPlanModal();
  bindEvents();
  if (window.__pendingScrollTop) {
    window.__pendingScrollTop = false;
    requestAnimationFrame(() => {
      window.scrollTo(0, 0);
      document.querySelectorAll('.content, .app-frame, .filter-body').forEach(el => { try { el.scrollTop = 0; } catch (_) {} });
    });
  }
}
...
function migrateState() {
  ...
  state.sort = state.sort || 'Best Match';
  state.search = state.search || '';
  state.finder = state.finder || {};
  if (state.finder.purpose && !state.finder.category) state.finder.category = categoryFromPurpose(state.finder.purpose);
  if (!state.finder.category) state.finder.category = 'Quick and simple';
  if (!state.finder.materials) state.finder.materials = 'No Materials';
  state.games.forEach(g => {
    if (!Array.isArray(g.categories)) g.categories = normalizeCategories([...(g.categories || []), ...(g.purpose || []), ...(g.tags || [])].join(','));
    g.tags = [...new Set([...(Array.isArray(g.tags) ? g.tags : []), ...g.categories, ...(g.materials.length ? g.materials.map(titleCaseMaterial) : ['No Materials'])])];
  });
}
function categoryFromPurpose(value = '') {
  const text = String(value).toLowerCase();
  if (text.includes('water') || text.includes('wet')) return 'Wet-n-Wild';
  if (text.includes('team')) return 'Team-building';
  if (text.includes('ice')) return 'Icebreakers';
  if (text.includes('tag')) return 'Tag';
  if (text.includes('relay')) return 'Relays';
  if (text.includes('sport') || text.includes('fitness')) return 'Sport-n-fitness';
  if (text.includes('circle')) return 'Circle';
  return 'Quick and simple';
}
function normalizeCategories(value = '') {
  const text = String(Array.isArray(value) ? value.join(',') : value).toLowerCase();
  const picked = STANDARD_CATEGORIES.filter(c => text.includes(c.toLowerCase()));
  if (text.includes('teamwork') || text.includes('communication') || text.includes('trust')) picked.push('Team-building');
  if (text.includes('icebreaker')) picked.push('Icebreakers');
  if (text.includes('no prep') || text.includes('quick') || text.includes('fun')) picked.push('Quick and simple');
  return [...new Set(picked)].slice(0, 4).length ? [...new Set(picked)].slice(0, 4) : ['Quick and simple'];
}
function titleCaseMaterial(value = '') {
  if (!value) return '';
  if (String(value).toLowerCase() === 'no materials') return 'No Materials';
  return String(value).trim().replace(/\b\w/g, m => m.toUpperCase());
}
...
function renderTopbar(title = '') {
  return `<header class="topbar"><button class="icon-btn back-btn" data-back aria-label="Back"><img src="/assets/back_button.png" alt=""></button><div class="header-brand">${title ? `<div class="topbar-title">${title}</div>` : APP_HEADER}</div><div class="header-actions"><button class="icon-btn asset-icon" data-open-sort title="Sort" aria-label="Sort"><img src="/assets/sort.png" alt=""></button><button class="icon-btn asset-icon" data-go="/filter" title="Filter" aria-label="Filter"><img src="/assets/filter.png" alt=""></button></div></header>`;
}

function renderAppShell(inner, opts = {}) {
  return `<div class="app-frame">${!navigator.onLine ? '<div class="offline-banner">Offline mode: payments, uploads, and submissions are disabled.</div>' : ''}${renderTopbar(opts.title || '')}${inner}${renderBottomNav()}</div>`;
}
...
function renderAppRoute() {
  ...
  if (p[1] === 'plan' && p[2] && p[3] === 'modify') return renderModifyPlan(p[2]);
  if (p[1] === 'plan' && p[2] === 'materials') return renderMaterialSelector('/app/plan');
...
function getFilteredGames() {
  ...
  if (q) games = games.filter(g => [g.title,g.shortDescription,g.description,g.materials.join(' '),g.tags.join(' '),(g.categories||[]).join(' ')].join(' ').toLowerCase().includes(q));
  ...
  filters.forEach(f => {
    ...
    else games = games.filter(g => g.tags.map(x=>x.toLowerCase()).includes(n) || (g.categories||[]).map(x=>x.toLowerCase()).includes(n) || g.materials.map(x=>x.toLowerCase()).includes(n));
  });
...
function scoreGames(games = state.games) {
  const f = state.finder;
  const materials = selectedMaterialsArray(f.materials).map(x=>x.toLowerCase());
  return games.map(game => {
    ...
    else if (materials.includes('no materials')) score -= 20;
...
function renderGameDetail(id) {
  ...
  if (!game) return `<div class="app-frame">${renderTopbar()}<main class="content"><h1>Game not found</h1></main></div>`;
  game.views += 1; saveState();
  const locked = !canViewGame(game);
  return `<div class="app-frame game-page-frame">${renderGameTopbar(game.id)}<div class="detail-title-block"><h1>${game.title}</h1><div class="rating-row" style="justify-content:flex-start;gap:12px">${stars(game.averageRating)} <span>${game.averageRating.toFixed(1)} (${game.ratingCount} Ratings)</span><button class="icon-btn heart ${isFavorite(game.id)?'active':''}" data-toggle-favorite="${game.id}">♥</button></div><p><a class="link" data-go="/app/games/${game.id}/reviews">View Reviews</a></p></div><main class="content" style="padding-top:0;padding-bottom:132px"><div class="media-hero thumb ${game.thumb}"><span>${game.title}</span></div><div class="dots"><span class="dot"></span><span class="dot active"></span><span class="dot"></span></div><section class="content-section"><h2>Description:</h2><p>${game.description}</p></section><section class="content-section"><h2>Quick Facts:</h2><div class="quick-facts">${quickFacts(game).map(([k,v])=>`<div class="fact"><strong>${k}</strong><span>${v}</span></div>`).join('')}</div></section>${locked ? renderLockedGame(game) : renderFullGameDetails(game)}<section class="content-section"><h2>Filters:</h2><div class="tag-list">${game.tags.map(t=>`<span class="tag">${t}</span>`).join('')}</div></section><section class="content-section"><h2>Creators:</h2><p>${game.creator}</p></section></main><div class="sticky-actions three-actions"><button class="btn btn-primary" data-go="/app/games/${game.id}/rate">Rate This Game</button><button class="btn btn-secondary" data-go="/app/games/${game.id}/notes">Add Notes</button><button class="btn btn-secondary" data-add-to-plan="${game.id}">Add to Plan</button></div></div>`;
}
function renderGameTopbar(gameId) {
  return `<header class="topbar"><button class="icon-btn back-btn" data-back aria-label="Back"><img src="/assets/back_button.png" alt=""></button><div class="header-brand">${APP_HEADER}</div><div class="header-actions"><button class="icon-btn asset-icon" data-share-game="${gameId}" title="Share" aria-label="Share"><img src="/assets/ios-share.svg" alt=""></button></div></header>`;
}
...
function renderRateScreen(id) {
  ...
  return `<div class="app-frame fullscreen-page game-page-frame">${renderGameTopbar(id)}<main class="content no-bottom-nav"><form id="rate-form" data-game-id="${id}"><input type="hidden" name="rating" value="${existing.rating||0}"><div class="rate-card star-picker">${[1,2,3,4,5].map(i=>`<button type="button" class="${(existing.rating||0)>=i?'active':''}" data-star="${i}">☆</button>`).join('')}</div><label><span class="label">Review</span><textarea name="reviewText" placeholder="Write review">${escapeHTML(existing.reviewText||'')}</textarea></label><h2>IMAGES</h2><label class="upload-tile">+<input name="images" type="file" accept="image/*" multiple hidden></label><h2>VIDEOS</h2><label class="upload-tile">+<input name="videos" type="file" accept="video/*" multiple hidden></label></form></main><div class="sticky-actions" style="grid-template-columns:1fr"><button class="btn btn-primary full" form="rate-form">Submit</button></div></div>`;
}
...
function renderPlan() {
  if (!hasProAccess()) return `<main class="content"><div class="lock-card"><h1>My Plans is PRO</h1><p>Build, save, reopen, and manage full session plans.</p><button class="btn btn-primary full" data-go="/pricing">Upgrade to PRO</button></div></main>`;
  const plans = userPlans();
  return `<main class="content"><div class="plans-page-head"><div><h1>My Plans</h1><p class="help">View saved plans, modify games, rename, or remove old plans.</p></div><button class="create-plan-btn" data-create-plan><span>+</span><strong>Create a New Plan</strong></button></div><section class="game-list">${plans.map(renderSessionPlan).join('') || '<div class="card" style="padding:22px"><h2>No saved plans yet.</h2><p class="help">Use Create a New Plan or add games from the Find screen.</p></div>'}</section></main>`;
}
function renderSessionPlan(plan) {
  const items = planItemsWithGames(plan);
  return `<section class="card plan-output" style="padding:18px"><h2>${escapeHTML(plan.title)}</h2><p class="help">Combined materials: ${planMaterials(plan).join(', ') || 'none'}</p>${items.map(item=>`<button class="timeline-item clickable-timeline" data-go="/app/games/${item.gameId}"><div class="timeline-time">${escapeHTML(item.start || '0:00')} · ${item.duration || 10} min</div><div class="timeline-title">${escapeHTML(item.type || 'Game')}: ${escapeHTML(item.title)}</div><p class="help">${escapeHTML(item.notes || '')}</p></button>`).join('')}<div class="plan-actions"><button class="btn btn-secondary" data-modify-plan="${plan.id}"><img class="btn-icon" src="/assets/edit-246.svg" alt="">Modify</button><button class="btn btn-secondary" data-rename-plan="${plan.id}">Rename</button><button class="btn btn-danger" data-delete-plan="${plan.id}">Delete</button></div></section>`;
}


function userPlans() { return state.sessions.filter(s => s.userId === currentUser().id); }
function planItemsWithGames(plan) {
  return (plan.items || []).map((item, index) => {
    const game = item.gameId ? state.games.find(g => g.id === item.gameId) : state.games.find(g => g.title === item.title);
    return { ...item, gameId: game?.id || item.gameId || '', title: item.title || game?.title || `Game ${index+1}`, notes: item.notes || game?.shortDescription || '' };
  }).filter(item => item.gameId);
}
function planMaterials(plan) {
  const ids = planItemsWithGames(plan).map(item => item.gameId);
  return [...new Set(ids.flatMap(id => (state.games.find(g=>g.id===id)?.materials || [])))];
}
function createBlankPlan(title = 'New Plan', gameId = null) {
  const plan = { id: crypto.randomUUID(), userId: currentUser().id, title, materials: [], items: [] };
  if (gameId) addGameToPlanObject(plan, gameId);
  state.sessions.push(plan);
  saveState();
  return plan;
}
function addGameToPlanObject(plan, gameId) {
  const game = state.games.find(g => g.id === gameId);
  if (!game) return;
  plan.items = plan.items || [];
  if (plan.items.some(item => item.gameId === gameId)) return;
  plan.items.push({ type: plan.items.length ? 'Game' : 'Main Game', title: game.title, gameId: game.id, start: `${plan.items.length * 10}:00`, duration: Math.max(5, game.timeMin || 10), notes: game.shortDescription });
  plan.materials = planMaterials(plan);
}
function renderModifyPlan(planId) {
  const plan = state.sessions.find(p => p.id === planId && p.userId === currentUser().id);
  if (!plan) return `<main class="content"><div class="card" style="padding:22px"><h1>Plan not found</h1></div></main>`;
  const items = planItemsWithGames(plan);
  return `<main class="content"><section class="hero-card"><h1>Modify Plan</h1><p>${escapeHTML(plan.title)} — drag games up or down, or swipe left to remove.</p></section><div class="modify-list" data-plan-id="${plan.id}">${items.map((item, index)=>renderSwipePlanItem(plan.id, item, index)).join('') || '<div class="card" style="padding:22px"><h2>No games in this plan yet.</h2></div>'}</div></main>`;
}
function renderSwipePlanItem(planId, item, index) {
  return `<div class="swipe-item" data-plan-id="${planId}" data-game-id="${item.gameId}" draggable="true"><div class="swipe-action" aria-hidden="true"><span>Remove</span></div><div class="swipe-card card plan-game-card" data-go="/app/games/${item.gameId}"><div class="drag-handle" aria-hidden="true">☰</div><div><div class="timeline-time">${index + 1}</div><div class="timeline-title">${escapeHTML(item.title)}</div><p class="help">${escapeHTML(item.notes || '')}</p></div><button class="sr-remove-btn" data-remove-plan-game="${planId}:${item.gameId}" aria-label="Remove ${escapeHTML(item.title)}">Remove</button></div></div>`;
}
function renderMaterialSelector(backPath = '/app/plan') {
  const selected = selectedMaterialsArray(state.finder.materials);
  return `<div class="app-frame fullscreen-page">${renderTopbar('AVAILABLE MATERIALS')}<main class="filter-body"><section class="filter-group"><h2>Choose what you have</h2><div class="filter-chip-grid material-cloud">${MATERIAL_OPTIONS.map(o=>`<button class="filter-chip ${selected.includes(o)?'active':''}" data-material-choice="${o}">${o}</button>`).join('')}</div></section></main><div class="filter-bottom"><button class="btn btn-primary" data-material-clear>No Materials</button><button class="btn btn-secondary" data-go="${backPath}">Done</button></div></div>`;
}
...
function renderAccount() {
  const u = currentUser();
  const batchCard = isStaff(u) ? `<div class="card" style="padding:18px"><h2>Staff Batch Import</h2><p class="help">Upload multiple Markdown game files from the Add Games screen.</p><button class="btn btn-secondary full" data-go="/app/submit">Open Add Games</button></div>` : '';
  return `<main class="content"><section class="hero-card"><h1>Account</h1><p>${u.fullName} · ${u.email}</p><div class="magic-row"><span class="magic-chip">Role: ${u.role}</span><span class="magic-chip">Plan: ${u.plan}</span><span class="magic-chip">Access: ${hasProAccess(u)?'PRO active':'Free'}</span></div></section><div class="game-list"><div class="card" style="padding:18px"><h2>My Plans</h2><p class="help">Build, save, view, rename, modify, and reopen your saved plans.</p><button class="btn btn-primary full" data-go="/app/plan">Open My Plans</button></div><div class="card" style="padding:18px"><h2>Redeem Coupon</h2><form id="redeem-form" class="form-grid"><input class="input" name="code" maxlength="16" placeholder="16-character code"><button class="btn btn-primary full">Redeem one free month</button></form></div>${batchCard}<button class="btn btn-secondary full" data-go="/pricing">Upgrade / Manage Billing</button>${isStaff(u)?'<button class="btn btn-dark full" data-go="/admin">Admin Panel</button>':''}<button class="btn btn-danger full" data-logout>Log out</button></div></main>`;
}

function renderAdminRoute() {
  if (!isStaff()) return `<div class="app-frame">${renderTopbar('ADMIN')}<main class="content"><div class="lock-card">Only staff can access admin tools.</div></main></div>`;
  const sub = route.parts[1] || 'dashboard';
  const title = sub === 'dashboard' ? 'Admin' : `Admin > ${sub[0].toUpperCase()+sub.slice(1)}`;
  const inner = sub === 'users' ? renderAdminUsers() : sub === 'coupons' ? renderAdminCoupons() : sub === 'submissions' ? renderAdminSubmissions() : sub === 'games' ? renderAdminGames() : renderAdminDashboard();
  return `<div class="app-frame">${renderTopbar(title)}<main class="content"><div class="chip-scroll"><button class="chip quick-chip" data-go="/admin">Dashboard</button><button class="chip quick-chip" data-go="/admin/users">Users</button><button class="chip quick-chip" data-go="/admin/coupons">Coupons</button><button class="chip quick-chip" data-go="/admin/submissions">Submissions</button><button class="chip quick-chip" data-go="/admin/games">Games</button></div>${inner}</main>${renderBottomNav()}</div>`;
}
...
function bindEvents() {
  document.querySelectorAll('[data-go]').forEach(el => el.addEventListener('click', e => { e.preventDefault(); e.stopPropagation(); go(el.dataset.go); }));
  document.querySelectorAll('[data-back]').forEach(el => el.addEventListener('click', e => { e.preventDefault(); e.stopPropagation(); goBack(); }));
  ...
  document.querySelectorAll('[data-open-sort]').forEach(el => el.addEventListener('click', e => { e.preventDefault(); e.stopPropagation(); window.__sortOpen = true; render(); }));
  ...
  document.querySelectorAll('[data-add-to-plan]').forEach(el => el.addEventListener('click', e => { e.preventDefault(); e.stopPropagation(); window.__addToPlanGameId = el.dataset.addToPlan; render(); }));
  document.querySelectorAll('[data-close-plan-modal]').forEach(el => el.addEventListener('click', () => { window.__addToPlanGameId = null; render(); }));
  document.querySelectorAll('[data-add-to-existing-plan]').forEach(el => el.addEventListener('click', () => addGameToExistingPlan(el.dataset.addToExistingPlan, window.__addToPlanGameId)));
  const createFromGame = byId('create-plan-from-game-form'); if (createFromGame) createFromGame.addEventListener('submit', handleCreatePlanFromGame);
  document.querySelectorAll('[data-create-plan]').forEach(el => el.addEventListener('click', handleCreatePlan));
  document.querySelectorAll('[data-modify-plan]').forEach(el => el.addEventListener('click', () => go(`/app/plan/${el.dataset.modifyPlan}/modify`)));
  ...
  document.querySelectorAll('[data-remove-plan-game]').forEach(el => el.addEventListener('click', e => { e.preventDefault(); e.stopPropagation(); const [planId, gameId] = el.dataset.removePlanGame.split(':'); removeGameFromPlan(planId, gameId); }));
  document.querySelectorAll('[data-material-choice]').forEach(el => el.addEventListener('click', () => toggleMaterialChoice(el.dataset.materialChoice)));
  document.querySelectorAll('[data-material-clear]').forEach(el => el.addEventListener('click', () => { state.finder.materials = 'No Materials'; saveState(); render(); }));
...
function renderAddToPlanModal() {
  const gameId = window.__addToPlanGameId;
  if (!gameId) return '';
  const game = state.games.find(g => g.id === gameId);
  if (!game) return '';
  const plans = userPlans();
  return `<div class="modal-backdrop" data-close-plan-modal><div class="bottom-sheet" onclick="event.stopPropagation()"><h2>Add to Plan</h2><p class="help">${escapeHTML(game.title)}</p><div class="plan-choice-list">${plans.map(plan=>`<button class="plan-choice" data-add-to-existing-plan="${plan.id}"><strong>${escapeHTML(plan.title)}</strong><span>${planItemsWithGames(plan).length} game(s)</span></button>`).join('') || '<p class="help">No saved plans yet. Create one below.</p>'}</div><form id="create-plan-from-game-form" class="form-grid" style="margin-top:14px"><label><span class="label">Create New Plan</span><input class="input" name="title" value="New Plan"></label><button class="btn btn-primary full">Create and Add</button></form><button class="btn btn-secondary full" style="margin-top:12px" data-close-plan-modal>Cancel</button></div></div>`;
}
function addGameToExistingPlan(planId, gameId) {
  const plan = state.sessions.find(p => p.id === planId && p.userId === currentUser().id);
  if (!plan) return toast('Plan not found.');
  addGameToPlanObject(plan, gameId);
  saveState();
...
function handleCreatePlanFromGame(e) {
  e.preventDefault();
  const title = new FormData(e.target).get('title') || 'New Plan';
  const plan = createBlankPlan(title, window.__addToPlanGameId);
  window.__addToPlanGameId = null;
  toast('Plan created.');
  go(`/app/plan/${plan.id}/modify`);
}
function handleCreatePlan() {
  const title = window.prompt('Plan name', 'New Plan');
  if (!title) return;
  const plan = createBlankPlan(title);
  toast('Plan created.');
  go(`/app/plan/${plan.id}/modify`);
}
function renamePlan(planId) {
  const plan = state.sessions.find(p => p.id === planId && p.userId === currentUser().id);
  if (!plan) return;
  const title = window.prompt('Rename plan', plan.title);
  if (!title) return;
  plan.title = title;
  saveState();
  render();
...
function deletePlan(planId) {
  state.sessions = state.sessions.filter(p => !(p.id === planId && p.userId === currentUser().id));
  saveState();
  toast('Plan removed.');
  render();
}
function removeGameFromPlan(planId, gameId) {
  const plan = state.sessions.find(p => p.id === planId && p.userId === currentUser().id);
  if (!plan) return;
  plan.items = (plan.items || []).filter(item => (item.gameId || state.games.find(g=>g.title===item.title)?.id) !== gameId);
  plan.materials = planMaterials(plan);
  saveState();
  render();
}
function toggleMaterialChoice(material) {
  if (material === 'No Materials') {
    state.finder.materials = 'No Materials';
  } else {
    const list = selectedMaterialsArray(state.finder.materials).filter(x => x !== 'No Materials');
    const next = list.includes(material) ? list.filter(x => x !== material) : [...list, material];
    state.finder.materials = next.length ? next.join(', ') : 'No Materials';
  }
  saveState();
  render();
}
function bindPlanReorder() {
  ...
  document.querySelectorAll('.swipe-item[draggable="true"]').forEach(item => {
    item.addEventListener('dragstart', e => { dragGameId = item.dataset.gameId; item.classList.add('dragging'); e.dataTransfer.effectAllowed = 'move'; });
    item.addEventListener('dragend', () => item.classList.remove('dragging'));
    item.addEventListener('dragover', e => { e.preventDefault(); });
    item.addEventListener('drop', e => {
      e.preventDefault();
      const targetGameId = item.dataset.gameId;
      const planId = item.dataset.planId;
      if (!dragGameId || dragGameId === targetGameId) return;
      reorderPlanGame(planId, dragGameId, targetGameId);
    });
  });
}
function reorderPlanGame(planId, dragGameId, targetGameId) {
  const plan = state.sessions.find(p => p.id === planId && p.userId === currentUser().id);
  if (!plan) return;
  const items = planItemsWithGames(plan);
  const from = items.findIndex(item => item.gameId === dragGameId);
  const to = items.findIndex(item => item.gameId === targetGameId);
...
function bindSwipeRemoval() {
  let openCard = null;
  document.querySelectorAll('.swipe-item').forEach(item => {
    const card = item.querySelector('.swipe-card');
    if (!card) return;
    let startX = 0, startY = 0, dx = 0, active = false, swiping = false, blockedClick = false;
    card.addEventListener('pointerdown', e => {
      if (e.target.closest('button')) return;
      if (openCard && openCard !== card) resetSwipeCard(openCard);
      openCard = card;
      startX = e.clientX; startY = e.clientY; dx = 0; active = true; swiping = false; blockedClick = false;
      card.classList.add('dragging-swipe');
    });
    card.addEventListener('pointermove', e => {
      ...
      if (!swiping) {
        if (Math.abs(moveY) > Math.abs(moveX) && Math.abs(moveY) > 8) { active = false; card.classList.remove('dragging-swipe'); return; }
        if (moveX < -8 && Math.abs(moveX) > Math.abs(moveY)) swiping = true;
      }
      if (!swiping) return;
      e.preventDefault();
      blockedClick = true;
      dx = Math.min(0, moveX);
      card.style.transform = `translateX(${Math.max(dx, -card.offsetWidth)}px)`;
    }, { passive: false });
    const finish = () => {
      if (!active) return;
      active = false;
      card.classList.remove('dragging-swipe');
      if (!swiping) return;
      const threshold = Math.max(120, card.offsetWidth * 0.4);
      if (Math.abs(dx) >= threshold) {
        card.classList.add('removing');
        card.style.transform = `translateX(-${card.offsetWidth}px)`;
        item.style.maxHeight = `${item.offsetHeight}px`;
        setTimeout(() => { item.style.maxHeight = '0px'; item.style.opacity = '0'; item.style.margin = '0'; }, 120);
        setTimeout(() => removeGameFromPlan(item.dataset.planId, item.dataset.gameId), 330);
      } else {
        resetSwipeCard(card);
      }
    };
    ...
    card.addEventListener('click', e => {
      if (blockedClick) { e.preventDefault(); e.stopPropagation(); blockedClick = false; }
    }, true);
  });
}
...
function handleFinder(e) {
  ...
  state.finder = { ...state.finder, groupSize: Number(fd.get('groupSize')), time: Number(fd.get('time')), ageMin: Number(fd.get('ageMin')), ageMax: Number(fd.get('ageMax')), space: fd.get('space'), category: fd.get('category'), safety: 'Low Risk', avoid: 'not embarrassing' };
  saveState();
  toast('Smart Finder updated your top matches.');
  render();
}
...
function shareGame(id) {
  const url = `${location.origin}${location.pathname}#/app/games/${id}`;
